<?php
  ...
  function has_unique_manager_username($username, $current_id="0") {
    global $db;
    ...
    $manager_set = mysqli_query($db, $sql);
    $manager_count = mysqli_num_rows($manager_set);
    mysqli_free_result($manager_set);
  ...
  function has_unique_customer_username($username, $current_id="0") {
    ...
    $sql .= "WHERE username='" . db_escape($db, $username) . "' ";
    $sql .= "AND customer_id != '" . db_escape($db, $current_id) . "'";

    $customer_set = mysqli_query($db, $sql);
    $customer_count = mysqli_num_rows($customer_set);
    mysqli_free_result($customer_set);

    return $customer_count === 0;
  }
  function has_unique_vehicles_vin($vin) {
    global $db;

    $sql = "SELECT * FROM vehicles ";
    $sql .= "WHERE vin='" . db_escape($db, $vin) . "'";

    $vehicle_set = mysqli_query($db, $sql);
    $vehicle_count = mysqli_num_rows($vehicle_set);
    mysqli_free_result($vehicle_set);

    return $vehicle_count === 0;
  }
  function has_unique_drivers_l($l) {
    global $db;

    $sql = "SELECT * FROM drivers ";
    $sql .= "WHERE license_number='" . db_escape($db, $l) . "'";

    $driver_set = mysqli_query($db, $sql);
    $driver_count = mysqli_num_rows($driver_set);
    mysqli_free_result($driver_set);
